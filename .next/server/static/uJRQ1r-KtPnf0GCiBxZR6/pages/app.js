...
/* harmony default export */ var app = __webpack_exports__["default"] = (App); //Servidor que recebe a validação da conexão, não vai para o frontEnd
...
async function getServerSideProps({
  ...
  if (session) {
    ...
    const todaysCheckin = await db["db"].collection("markers").doc(currentDate).collection("checks").doc(session.user.sub).get();
    ...
    if (todaysData) {
      ...
      const checkins = await db["db"].collection('markers').doc(currentDate).collection('checks') //verifica os próximos
      .near({
      ...
      checkins.docs.forEach(doc => {
        checkinsList.push({
          ...
          distance: distance(todaysData.coordinates.latitude, todaysData.coordinates.longitude, doc.data().coordinates.latitude, doc.data().coordinates.loangitude)
        });
      });
      return {
        props: {
